...
pgsp_shmem_startup(void)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		entry->n_relids = temp.n_relids;
  		memcpy(entry->relids, temp.relids, temp.n_relids * sizeof(Oid));                                                                                                      
...
pgsp_create_entry_if_not_exists(QueryDesc *queryDesc)
{
	...
	json_plan = pgsp_build_json_plan(queryDesc, false);
	Assert(json_plan != NULL);
		   ...
		   const BufferUsage *bufusage)
{
	...
	if (!entry)
	{
		json_plan = pgsp_build_json_plan(queryDesc, true);
